...
if __name__=="__main__":
    cid=295
    input_folder='/Users/icasdb/Desktop/nccts_for_registration_2thickness/'+str(cid)+'/'
    dcm_list=glob.glob(input_folder+'*/*/*dcm')
    full_sorted_dcm_list, dcm_thickness_lists,thickness_list=dicom_sorter_thickness_splitter(dcm_list)
    ...
    zlowercoverage=thickness_list[0]*len(lower_dcm_list)
    zuppercoverage=thickness_list[1]*len(upper_dcm_list)
#    zn=dicom.read_file(full_sorted_dcm_list[len(full_sorted_dcm_list)-1])
#    zcoverage=float(zn.SliceLocation)-float(z1.SliceLocation)+(A_zspacing/2)+(B_zspacing/2)
    zcoverage=zlowercoverage+zuppercoverage
...
#    new_zcoverage=math.ceil(zcoverage/B_zspacing)
    new_num_slices=round(zcoverage/thickness_list[1])
    transformParameterMap[0]["Spacing"]=(str(B_pixelspacing),str(B_pixelspacing),str(B_zspacing))
    transformParameterMap[0]["Size"]=('512','512',str(new_num_slices))
